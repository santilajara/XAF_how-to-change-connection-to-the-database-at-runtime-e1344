...
Namespace RuntimeDbChooser.Web
	Partial Public Class RuntimeDbChooserAspNetApplication
		...
		Protected Overrides Sub ReadSecuredLogonParameters()
			...
			End If
		End Sub
'INSTANT VB NOTE: The field canReadSecuredLogonParameters was renamed since Visual Basic does not allow fields to have the same name as other class members:
		Private canReadSecuredLogonParameters_Conflict As Boolean = True
		Protected Overrides Function CanReadSecuredLogonParameters() As Boolean
			If Not canReadSecuredLogonParameters_Conflict Then
				Return False
			End If
		...
		Protected Overrides Function OnLogonFailed(ByVal logonParameters As Object, ByVal e As Exception) As Boolean
			If CanReadSecuredLogonParameters() Then
				...
				canReadSecuredLogonParameters_Conflict = False
				Try
					Start()
				Finally
					canReadSecuredLogonParameters_Conflict = True
				End Try
				Return True
			Else
